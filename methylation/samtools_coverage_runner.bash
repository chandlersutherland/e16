...
coverage_input=/global/scratch/users/chandlersutherland/e16/${sample}/em/bismark
cd $coverage_input
bams=$(find . -type f -name '*.bam')

#input is a bam file 
processing () {
	#first, convert to bam, sort and index for samtools coverage to run appropriately 
	basename=$(basename ${1} .bam) 
	samtools sort -o $basename.sorted.bam ${1} 
	...
	python $HOME/e16/methylation/samtools_coverage.py  $NLR_bed $basename.sorted.bam 
